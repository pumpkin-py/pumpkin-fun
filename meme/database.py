...
class HugEmote(database.base):
    """HugEmote

    Configuration for hugging emote
    
    """

    __tablename__ = "fun_meme_hugemote"
    ...
    @staticmethod
    def change(guild_id: int, emote: string) -> HugEmote:
        """Set hug emote

        :return: Relation.
        """

        query = session.query(HugEmote).filter_by(guild_id=guild_id).first()
    ...
    def get(guild_id: int) -> HugEmote:
    
        query = session.query(HugEmote).filter_by(guild_id=guild_id).one_or_none()
